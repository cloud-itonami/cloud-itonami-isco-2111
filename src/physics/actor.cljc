(ns physics.actor
  "ResearchActor — the ISCO-08 2111 research support actor as a
  `langgraph.graph/state-graph` (per ADR-2607011000 / CLAUDE.md
  Actors section). One graph run = one research operation request
  ...
  The unconditional invariant: the ResearchAdvisor can never
  directly commit a record or dispatch a robot action the
  ResearchGovernor refuses — every commit-record! call is gated
  behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [physics.advisor :as advisor]
            [physics.governor :as governor]
            [physics.store :as store]))

(defn build-graph
  "Build a compiled ResearchActor graph. `store` implements
  `physics.store/Store`. `advisor` implements
  `physics.advisor/Advisor` (defaults to `mock-advisor`).
...
(defn run-request!
  "Run one research operation request to completion or interrupt.
